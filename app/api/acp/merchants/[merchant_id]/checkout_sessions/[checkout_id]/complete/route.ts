...
export async function POST(
  ...
  try {
    ...
    const squareConnections = connections.filter(
      (conn) => conn.provider === "square",
    );

    console.log(
      `🤖 [ACP-COMPLETE] Found ${squareConnections.length} Square connections`,
    );
    ...
    for (const connection of squareConnections) {
      try {
        ...
        if (!provider || !provider.createOrder) {
          console.log(
            "🤖 [ACP-COMPLETE] Square provider doesn't support order creation",
          );
          continue;
        }
        ...
        if (lineItems.length === 0) {
          ...
          try {
            ...
            console.log(
              "🤖 [ACP-COMPLETE] Error fetching product data for fallback:",
              error,
            );
          }
        ...
        console.log(
          "🤖 [ACP-COMPLETE] Using checkout session data:",
        ...
        const squareLineItems = checkoutSessionData.line_items.map(
          (lineItem: any) => {
        ...
        console.log(
          "🤖 [ACP-COMPLETE] Transformed Square line items:",
          JSON.stringify(squareLineItems, null, 2),
        );

        // Extract totals from checkout session
        const taxAmount =
          checkoutSessionData.totals.find((t: any) => t.type === "tax")
            ?.amount || 0;
        const checkoutTotal =
          checkoutSessionData.totals.find((t: any) => t.type === "total")
            ?.amount || 0;

        console.log(
          "🤖 [ACP-COMPLETE] Checkout session tax amount:",
          taxAmount,
        );
        console.log(
          "🤖 [ACP-COMPLETE] Checkout session total amount:",
          checkoutTotal,
        );
    ...
    console.error(
      "🤖 [ACP-COMPLETE] Error processing Square connections:",
      error,
    );
  ...
  console.log(
    `✅ [ACP-COMPLETE] Marked checkout session ${checkout_id} as completed in memory`,
  );
